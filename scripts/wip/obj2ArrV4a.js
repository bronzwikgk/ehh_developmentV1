       ...
       // console.log("path here", path)
        ...
        for (var key in input) {
            ...
            if (getEntityType(input[key]) === 'Object') { // console.log("object",key,input[key])
               ...
               // console.log("processing", input[key], "at previous Row", previousRow, getEntityType(input[key]),input[key].length)
             ...
             //   console.log(input[key]);
              ...
              //  console.log("found value", key, input[key], currentRow, output);    
                //works with sample4
               // processData.updateRow(input[key], output, previous, d, currentKey, currentObj, currentRow, currentRow, previsouObj)
            //works for sample 1
        ...
        for (var i = 0; i < input.length; i++) {
            if (typeof input[i] === "object" && input[i] !== null) {
             // console.log("Found Object in Array", input[i],getEntityType(input[i]));
                ...
                var path = previousRow[5] + "." + previous;
               // console.log(path)
                var currentRow = processData.createRow(input[i], output, previous, d, i, currentObj, currentRow, previousRow, previsouObj, path);
    ...
    static Obj2(input, output, previous, d, currentKey, currentObj, currentRow, previousRow, previsouObj,path) { 
     ...
     //   console.log("path here >>>", path)
        ...
        switch (input?.constructor) {
            case Object:   
                if (!currentRow) {
                    currentRow = processData.createRow(input, output, previous, d, previous, currentObj, currentRow, previousRow, previsouObj, path)
                 //   console.log("found value", key, input[key], currentRow, output);
                    output.push(currentRow);
